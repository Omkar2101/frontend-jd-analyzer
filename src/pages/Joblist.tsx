...
//   const getBadgeClass = (
//     score: number,
//     scoreType: "bias" | "inclusivity" | "clarity"
//   ): string => {
...
//                         )}`}
//                       >
//                         Bias: {Math.round(100 * job.analysis.bias_score)}%
//                       </span>
//                       <span
//                         className={`badge ${getBadgeClass(
//                           job.analysis.inclusivity_score,
//                           "inclusivity"
//                         )}`}
//                       >
//                         Inclusivity:{" "}
//                         {Math.round(100 * job.analysis.inclusivity_score)}%
//                       </span>
//                       <span
//                         className={`badge ${getBadgeClass(
//                           job.analysis.clarity_score,
//                           "clarity"
//                         )}`}
//                       >
//                         Clarity: {Math.round(100 * job.analysis.clarity_score)}%
//                       </span>
//                     </div>
//                   </div>
//                 )}
...
//                     <i className="fas fa-eye me-1"></i>Click to view full
//                     analysis
//                   </small>
//                 </div>
//               </div>
...
const JobList: React.FC = () => {
  const [jobs, setJobs] = useState<Job[]>([]);
  const [error, setError] = useState<string | null>(null);
  const [fetchingJobs, setFetchingJobs] = useState<boolean>(false); // Add separate loading state
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      ...
      try {
        setError(null);
        setFetchingJobs(true); // Start loading
        ...
        setFetchingJobs(false); // End loading
      }
    };
  ...
  // === Loading State ===
  // Show loading if either auth is loading OR jobs are being fetched
  if (isLoading || fetchingJobs) {
    return (
      <div className="container loading-container">
        <div className="spinner-border text-primary loading-spinner" role="status">
          <span className="visually-hidden">Loading...</span>
        ...
        <p>
          {isLoading ? "Checking authentication..." : "Loading your job listings..."}
        </p>
      </div>
    );
  }
