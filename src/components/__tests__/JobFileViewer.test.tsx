...
interface MockDOMMatrixInterface {
  a: number;
  b: number;
  c: number;
  d: number;
  e: number;
  f: number;
  multiply: () => MockDOMMatrixInterface;
  translate: () => MockDOMMatrixInterface;
  scale: () => MockDOMMatrixInterface;
}

interface MockPath2DInterface {
  addPath: () => void;
  closePath: () => void;
  moveTo: () => void;
  lineTo: () => void;
...
beforeAll(() => {
  // Mock DOMMatrix with proper constructor signatures
  const MockDOMMatrix = vi.fn().mockImplementation((): MockDOMMatrixInterface => ({
    a: 1, b: 0, c: 0, d: 1, e: 0, f: 0,
    multiply: vi.fn().mockReturnThis(),
    translate: vi.fn().mockReturnThis(),
    scale: vi.fn().mockReturnThis()
  ...
  // Mock Path2D
  global.Path2D = vi.fn().mockImplementation((): MockPath2DInterface => ({
    addPath: vi.fn(),
    closePath: vi.fn(),
    moveTo: vi.fn(),
    lineTo: vi.fn()
  ...
  const MockImageData = vi.fn().mockImplementation((
    ...
    if (dataOrWidth instanceof Uint8ClampedArray) {
      ...
      return {
        ...
        width: width,
        height: height,
        colorSpace: (typeof heightOrSettings === 'object' ? heightOrSettings?.colorSpace : 'srgb')
      };
    }
  });
...
// Mock react-pdf completely
vi.mock('react-pdf', () => ({
  Document: ({ children, onLoadSuccess, loading, error }: PDFDocumentProps) => {
    React.useEffect(() => {
      // Simulate successful PDF load after mounting
      setTimeout(() => {
        if (onLoadSuccess) {
  ...
  Page: ({ pageNumber, onLoadSuccess }: PDFPageProps) => {
    React.useEffect(() => {
      if (onLoadSuccess) {
        onLoadSuccess({ pageNumber });
      }
...
// Mock react-doc-viewer
vi.mock('react-doc-viewer', () => ({
  default: ({ documents }: DocViewerProps) => (
    <div data-testid="doc-viewer">
      Text file content loaded from: {documents?.[0]?.uri || 'No URI'}
    </div>
  ),
