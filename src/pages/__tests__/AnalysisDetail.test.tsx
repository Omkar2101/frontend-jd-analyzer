...
beforeAll(() => {
  ...
  if (!(Promise as typeof Promise & { withResolvers?: unknown }).withResolvers) {
    (Promise as typeof Promise & {
      withResolvers: <T>() => PromiseWithResolvers<T>;
    }).withResolvers = function <T>(): PromiseWithResolvers<T> {
      let resolve: (value: T | PromiseLike<T>) => void;
  ...
  global.DOMMatrix = vi.fn().mockImplementation(() => mockDOMMatrixInstance) as unknown as typeof DOMMatrix;
  ...
  const MockImageData = vi.fn().mockImplementation((
    dataOrWidth: Uint8ClampedArray | number, 
    widthOrHeight?: number, 
    heightOrSettings?: number | ImageDataOptions, 
    ...
    if (dataOrWidth instanceof Uint8ClampedArray) {
      return {
        data: dataOrWidth,
        width: widthOrHeight || 1,
        height: typeof heightOrSettings === 'number' ? heightOrSettings : 1,
        colorSpace: (typeof heightOrSettings === 'object' ? heightOrSettings?.colorSpace : settings?.colorSpace) || 'srgb'
      };
    } else {
      const width = dataOrWidth;
      const height = widthOrHeight || 1;
      return {
        ...
        width: width,
        height: height,
        colorSpace: (typeof heightOrSettings === 'object' ? heightOrSettings?.colorSpace : undefined) || 'srgb'
      };
    }
  });
...
// Mock the JobFileViewer component completely to avoid PDF.js imports
interface MockJobFile {
  originalFileName?: string;
  fileName?: string;
}

vi.mock('../components/JobFileViewer', () => ({
  default: ({ job }: { job: MockJobFile }) => (
    <div data-testid="job-file-viewer">File: {job.originalFileName || job.fileName}</div>
  )
}));
...
vi.mock('react-doc-viewer', () => ({
  default: ({ documents }: { documents?: MockDocuments[] }) => (
    <div data-testid="doc-viewer">
      Document viewer mock: {documents?.[0]?.uri || 'No document'}
    </div>
  ),
...
describe('AnalysisDetail Page', () => {
  ...
  it('renders analysis detail with all sections when scores are not zero', async () => {
    (mockedAxios.get as jest.MockedFunction<typeof axios.get>).mockResolvedValueOnce({
      data: mockJobData
    });
  ...
  it('handles loading state', () => {
    // Mock a delayed response
    (mockedAxios.get as jest.MockedFunction<typeof axios.get>).mockImplementation(
      () => new Promise(resolve => setTimeout(resolve, 1000))
    );
  ...
  it('handles error state when API call fails', async () => {
    (mockedAxios.get as jest.MockedFunction<typeof axios.get>).mockRejectedValueOnce(
      new Error('API Error')
    );
